...
public static class QuotaTextFormatter
{
    ...
    private static string FormatWindowDuration(TimeSpan? duration, string cultureName, bool shortForm = false)
    {
        if (duration is null)
        {
            return shortForm ? "--" : (cultureName.StartsWith("zh", StringComparison.OrdinalIgnoreCase) ? "额度窗口" : "quota window");
        }
        ...
        if (minutes % 10080 == 0)
        {
            ...
            return shortForm ? $"{weeks}w" : cultureName.StartsWith("zh", StringComparison.OrdinalIgnoreCase) ? $"{weeks * 7}天窗口" : $"{weeks}w window";
        }

        if (minutes % 1440 == 0)
        {
            int days = (int)Math.Round(minutes / 1440);
            return shortForm ? $"{days}d" : cultureName.StartsWith("zh", StringComparison.OrdinalIgnoreCase) ? $"{days}天窗口" : $"{days}d window";
        }

        if (minutes % 60 == 0)
        {
            int hours = (int)Math.Round(minutes / 60);
            return shortForm ? $"{hours}h" : cultureName.StartsWith("zh", StringComparison.OrdinalIgnoreCase) ? $"{hours}小时窗口" : $"{hours}h window";
        }

        int roundedMinutes = (int)Math.Round(minutes);
        return shortForm ? $"{roundedMinutes}m" : cultureName.StartsWith("zh", StringComparison.OrdinalIgnoreCase) ? $"{roundedMinutes}分钟窗口" : $"{roundedMinutes}m window";
    }
}
